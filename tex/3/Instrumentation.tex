...
%The intensity and polarization of the light scattered by the electron is highly dependent on a number of geometric parameters, including $\chi$, and the angle subtended by the Sun $\Omega$. The two components of radiation received by the observer are the tangential component, $I_T$ in the plane perpendicular to the page, and radial component, $I_R$ in the plane of the page Figure~\ref{fig:omega}. T
...
%set of trigonometric expressions known as the van de Hulst coefficients.


%The geometry of the system is quite complex; it was first outlined by~\citet{minnaert1930} and expanded upon by~\citet{billings1966}. However, a few errors in \citet{billings1966} lead to many misconceptions about the scattering process and a full and correct treatment can now be found in \citep{billings1966}. The effects of the finite size of the Sun are incorporated into a set of trigonometric expressions known as the van de Hulst coefficients.


%Dealing with these two components of light must take into account that an electron receives light from across the entire solar disk. Therefore, the necessary angles to be taken into account are the angle the solar disk subtends as seen by the electron $\Omega$, and the angle $\chi $ between the sun center-electron line and the line-of-sight (LOS) Figure~\ref{fig:omega}. The geometry of the system is quite complex; it was first outlined by~\citet{minnaert1930} and expanded upon by~\citet{billings1966}. However, a few errors in \citet{billings1966} lead to many misconceptions about the scattering process and a full and correct treatment can now be found in \citep{howtap2009}. The tangential component ($I_T$), radial component ($I_R$), and polarization ($I_P$) of the scattered intensity are given by the expressions




%where $I_0$ is incident intensity, $\sigma_e$ is the electron scattering cross section, $z$ is the distance from scatterer to observer, $u$ is a limb darkening coefficient. $A$, $B$, $C$, and $D$ are the van de Hulst coefficients and are a trigonometric function only of the angle $\Omega$ subtended by the sun at the scattering point. 

%They describe trigonometrically the incident light received by the scattering agent from the entire solar disk. Equations (3.2--3.8) completely describe the light scattered by an electron at any position on the solar atmosphere, with the observer viewing the scattering at any angle. The intensities given by these equations are for a single electron. If there is a number of electrons $N_e$, then the intensity would simply be $I_{tot}N_e$; the intensity of scattered light from a point Q depends linearly on total number of scattering electrons at that point. Hence if we know the intensity of scattered light we may directly infer the total number electrons contributing to this intensity. This is allows a calculation of the total electron content of a CME, resulting in a mass calculation.
...
%the light scattered from this point is a maximum. 

%The scattering efficiency is {\it minimum} at an angle of $\chi = 90^{\circ}$, a fact that is often erroneously quoted in the literature. This is because the total scattered intensity at the locus of the point of $\chi = 90^{\circ}$ in the corona is actually a {\it maximum}. Along any line of sight, the Sun-Q vector is actually at its shortest at the locus of the point where $\chi = 90^{\circ}$. Since the distance from the Sun is minimum at this point, the incident intensity $I_0$ on the scattering electrons is maximum and the electron density is also maximum. These two facts mean that at $\chi=90^{\circ}$ the scattered intensity is a maximum, despite the efficiency of scattering being a minimum. Close to the Sun, the plane making an angle of $\chi = 90^{\circ}$ with the Sun-observer line is known as the plane-of-sky (POS), and is the plane of maximum scattered intensity. Any CME erupting close to the POS will be well observed. However, eruption of a CME at a large angle away from the POS will result in less scattered light from the CME electrons, making the CME appear quite faint. If the angle from the POS is unknown, there is no way of telling the amount by which the intensity is reduced. Observing a smaller intensity will result in us calculating a smaller number of electrons contributing to the scattered light. If no correction is made for the diminished intensity, the number of electrons and hence the total CME mass will be underestimated. The correction can only be made when the angle from the plane of sky is known. As if often the case, the angle is unknown and we must assume the CME lies on the plane of sky. If this is untrue, there will a proportion of the CME that we will not see, leading to an underestimation of its mass content by up to a factor of 2 \citep{vou00}.


%describes a sphere is space whose diameter is along the Sun-observer line. This sphere termed the Thomson surface, and its surface represents the points of maximum scattered light towards the observer. When a CME erupts from the Sun, the points in the CME closest to this sphere will have maximum scattered intensity. This makes the interpretation of interplanetary CME observations difficult. However, closer to the Sun, the sphere essentially lies on the plane of sky, the plane perpendicular to the Sun-observer line. If a CME lies close to this plane it will be well observed given that the scattered light from the CME will be a maximum. Hence CMEs that erupt from the limb are the best observed; we can be assured that we are observing the majority of the CMEs since all of its electrons will be scattering the maximum intensity light. Eruption at an angle away from the plane of sky will result in the scattered intensity of the CME diminishing. The CME electrons will scatter less light toward the observe and we observe less of the CME. If the angle from sky-plane is unknown it is impossible to determine how much of the CME is unobserved due to lack of scattered light. This leads to very large uncertainties when calculating CME mass (see Chapter 4).


%\begin{figure}[!t]
%\begin{center}
%\includegraphics[scale=1.1]{thomson_sphere.pdf}
%\caption[The Thomson sphere]{The Thomson sphere}
%\end{center}
%\end{figure}
